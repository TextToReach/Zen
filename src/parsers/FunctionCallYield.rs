use crate::{
	features::tokenizer::{InstructionEnum, TokenData, TokenTable, YieldInstructionEnum}, Debug, Print, PrintVec
};
use chumsky::prelude::*;
...
pub fn parser() -> Box<dyn Parser<TokenData, YieldInstructionEnum, Error = Simple<TokenData>>> {
	let out = Parsers::identifier()
		...
		.map(|(name, args)| YieldInstructionEnum::CallFunction { name: name.asIdentifier(), args: args });
