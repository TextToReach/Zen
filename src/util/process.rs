...
pub fn ExecuteBlock(scope_id: usize, manager: &mut ScopeManager, last_conditional_block: &mut ConditionalBlockType) {
	...
	for line in block.clone() {
		...
		InstructionEnum::Repeat { repeat_count, scope_pointer } => {
			for _ in num::range(0, repeat_count.floor() as i64) {
...
pub fn ProcessLine(line_feed: Vec<TokenData>, instr: (ParserOutput, InstructionEnum), current_scope_id: &mut usize, manager: &mut ScopeManager, last_conditional_block: &mut ConditionalBlockType) {
